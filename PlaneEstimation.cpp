...
float* EstimatePlaneImplicit(vector<Point3f> pts){
    ...
    // calculate the center of the gravity of the points
    //first calculate the sum of points
  for (int p = 0; p < num; p++) {
     sumx += pts[p].x;
     sumy += pts[p].y;
     sumz += pts[p].z;
  ...
  //second find t= sum of pionts/ number of points
 t.x = sumx/num;
 t.y = sumy / num;
 t.z = sumz / num;

  //printf("the center of the gravity of the points:");

 //cout << "t : " << t << endl; 

 // third remove t from the points
 for (int i = 0; i < num; i++) {
...
//            printf("NumInliers: %d \n",sampleResult.inliersNum);
    ...
    for (int idx=0;idx<num;idx++){
        if (bestResult.isInliers.at(idx)){
            ...
            outlierPts.push_back(pts.at(idx));
        }
    }
   // pts = outlierPts;
   // cout << "the points : " << num << " inliers " << inlierPts.size() << " outliers " << outlierPts.size() << endl;
   // pts = outlierPts;
    ...
    //--------------
  /*
float* SeqRANSAC(vector<Point3f> pts, float threshold , int iterateNum) {
    float* allplans[3];
    for (int i = 0; i < 3 ;i++) {
       
         allplans[i] = EstimatePlaneRANSAC(pts, threshold, iterateNum);
         pts = outlierPts;
         cout << "the points : " << " outliers " << outlierPts.size() << endl;
         i++;
         //return allplans[i];
    }
    cout << " SEQRANSAC  " << allplans[0] << "--" << allplans[1] << "--" << allplans[2] << endl;
    return allplans[0], allplans[1], allplans[2];
}
*/
